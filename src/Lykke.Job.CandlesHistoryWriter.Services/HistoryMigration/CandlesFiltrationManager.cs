...
namespace Lykke.Job.CandlesHistoryWriter.Services.HistoryMigration
{
    ...
    public class CandlesFiltrationManager
    {
        ...
        public bool Filtrate(ICandlesFiltrationRequest request)
        {
            ...
            if (Health != null && Health.State != CandlesFiltrationState.Finished)
                return false;
        ...
        private async Task DoFiltrateAsync(ICandlesFiltrationRequest request, CandlePriceType priceType)
        {
            ...
            var firstBiggestCandle =
                await _candlesHistoryRepository.TryGetFirstCandleAsync(request.AssetId, Constants.StoredIntervals.Last(),
                    priceType);
            if (firstBiggestCandle == null)
                return;
            
            var dateFrom = firstBiggestCandle.Timestamp; // The firsr candle in storage timestamp
            var dateTo = DateTime.UtcNow.AddDays(-DateTime.UtcNow.Day).TruncateTo(CandleTimeInterval.Day).AddDays(1); // The last day of the prevous month
            ...
            {
                if (candleToTest.Open * candleToTest.Close * candleToTest.High * candleToTest.Low < Constants.Epsilon)
                    return true;
            ...
            foreach (var candleBatch in candlesByInterval)
            {
                ...
                {
                    var countDeleted = await _candlesHistoryRepository.DeleteCandlesAsync(candleBatch.Value.AsEnumerable());
                    Health.DeletedCandlesCount += countDeleted;
                    continue;
                }

                var smallerInterval = Constants.StoredIntervals[k - 2]; // The previuos interval (k was incremented above)
                ...
                foreach (var candle in candleBatch.Value.AsEnumerable())
                {
                    ...
                    dateTo = candle.LastUpdateTimestamp.TruncateTo(smallerInterval).AddSeconds((int)smallerInterval);
